...
import ChipRadio from "./ChipRadio.js";
  ...
  const addTask = async (event) => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        TaskContract.addTask(task.taskText, task.imp, task.isDeleted)
          .then((response) => {
      ...
      {currentAccount === "" ? (
        ...
        <div className="App">
          <form style={{ margin: "20px 30px 20px" }}>
            ...
            />
            <ChipRadio setPriority={setPriority} />
          ...
          <ul>
            {tasks.map((item) => (
              <div key={item.id} className={`tasklist`}>
                <Task
                  classname={`bg-${item.imp}`}
